...
\begin{document}
\title{COMP0005 Algorithms: Group 6 Coursework Report}
\author{Arif Mohamad Sabri, Yan Slobodianik, Yahya Fahmy, Danyal Ahmed}
\date{March 2026}
\maketitle
...
\]
The insertion and search times of the algorithms are taken from the median of a batch of measurements across each of these checkpoints. The median was preferred over the mean as it reduces the effect of outliers produced by occasional timing spikes due to uncontrollable background OS processes. Results were demonstrated with a line graph using \texttt{matplotlib}.
...
To minimize "noise" and ensure the reliability of the \texttt{timeit} measurements, all experiments were conducted under a "clean boot" state on the local machine. All non-essential background applications, including web browsers, cloud synchronization services, and indexing tools, were manually terminated prior to execution.
...
Average search times under reverse-sorted inputs closely mirror the average search times under sorted inputs, which is expected as shown in Figure 6. LLRB tree generally has the lowest search times followed closely by the AVL tree with both measuring slightly more than $1.0 \times 10^{-6}$ seconds for 100, 000 elements. B Tree on the other hand, is significantly slower having an average search time of $4.0 \times 10^{-6}$ seconds at its peak at 50, 000 elements count (similar to sorted inputs) and just below $4.0 \times 10^{-6}$ seconds at 100, 000 element counts.
...
Based on the results, insert operations' performance are consistently topped by the LLRB tree, having the lowest average insertion times across the three data distributions. On the flipside, the AVL tree consistently performed the worst while the B Tree is overall in the middle despite having irregular spikes. The most prominent contributing factor towards the AVL tree's slow insertion time could be attributed to the fact that it checks for duplicates for every insert, which entails searching through the entire tree and adding $O(\log n)$ of cost which effectively doubles the work. The LLRB tree on the other hand checks for duplicates in its recursion process, making it more efficient. Overall however, all graphs closely show a trend of $O (\log n)$ growth which is expected of these data structures.
...
Overall, the LLRB tree is consistently the fastest tree for insertions across the three distributions. The AVL tree while suffering from duplicate checks in insert operations performs similarly to the LLRB tree in searching making the AVL and LLRB trees the better choices for in-memory search-heavy processes rather than the B Tree. While both the AVL and LLRB trees maintain $O(\log n)$ height, their empirical performance for inserts diverged due to the hidden facts. The AVL tree is "strictly" balanced (height difference $\leq 1$), which results in more frequent rotations during insertion compared to the "procrastinated" balancing of the LLRB. Despite poor performances within this experimental framework, in practical scenarios the B Tree is useful in processing large amounts of data in a node where $t$ is very large and utilizing disk-based storage to amortize the cost of scanning keys in a node by reading an entire node in a single operation~\cite{alma9931552389604761}. For an in-memory process with low values of $t$ as well as branching factors however, LLRB and AVL trees remain the better choices in terms of cost and time complexity.
...
\hline
Arif    &  25125738  & Ran experiments, analysed results and authored report & $25\%$ \\\hline
Yan     &25178418    &Developed experimentation framework, implemented BTree, reviewed report& $25\%$ \\\hline
Yahya   &   25010297 &  Implemented AVL Tree, iterated on experimentation framework, reviewed report   &  $25\%$   \\\hline
Danyal  &25021460    &Implemented Left-Leaning Red-Black Tree algorithm, iterated on experimentation framework, reviewed report     &$25\%$       \\
